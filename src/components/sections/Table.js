...
export const Table = (props) => {
  ...
  return (
    <center>
      <section className="lg:max-w-6xl mt-16 lg:h-screen h-auto w-full items-center justify-self-center flex-col gap-2 self-center flex">
        <Trailed
          isOpen
          wrapperStyle="lg:w-full text-start lg:px-0  lg:max-w-6xl max-w-md lg:mx-0 mx-auto h-auto flex justify-start items-start flex-col gap-2 px-4 mt-12 mb-2"
        >
          {headerProps?.map((val, idx) => (
            <React.Fragment key={val[idx]}>{val}</React.Fragment>
          ))}
        </Trailed>
        <NextUITable
          aria-label="Polonya'daki Üniversite Programları"
          isHeaderSticky
          ...
          className="lg:max-w-7xl mx-auto lg:px-0 px-4 my-8 lg:my-4"
